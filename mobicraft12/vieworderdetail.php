...
// Use the correct column name from the orders table
...
<table>
    ...
    <tbody>
        ...
        <tr class="app-row" data-id="<?php echo $row['application_number']; ?>">
            ...
            <td><?php echo $row['warranty_days']; ?></td>
            <td>
                <form method="post" action="vieworderdetail_update.php" style="display:flex; gap:6px; align-items:center;">
                    <input type="hidden" name="application_number" value="<?php echo $row['application_number']; ?>">
                    <select name="progress" style="padding:6px; background:#111; color:#fff; border:1px solid #444; border-radius:6px;">
                        <option value="Under Processing" <?php if(strtolower($row['progress'])==='under processing') echo 'selected'; ?>>Under Processing</option>
                        <option value="Out of Delivery" <?php if(strtolower($row['progress'])==='out of delivery') echo 'selected'; ?>>Out of Delivery</option>
                        <option value="Delivered" <?php if(strtolower($row['progress'])==='delivered') echo 'selected'; ?>>Delivered</option>
                    </select>
